...
public class RandomSelection implements ISelection {
	...
	public IndividualGroup[] Select(Population p, int groupCount) {
		...
		for (int k = 0; k < groupCount; k++) {
			int k1 = k; while (k1 == k) k1 = (int) Math.ceil(groupCount * Math.random()) - 1;
			int k2 = k; while (k2 == k) k2 = (int) Math.ceil(groupCount * Math.random()) - 1;
			int k3 = k; while (k3 == k) k3 = (int) Math.ceil(groupCount * Math.random()) - 1;
