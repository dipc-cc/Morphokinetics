 ...
 * @author karmele
 */
public abstract class CatalysisKmc extends AbstractKmc {

  private CatalysisLattice lattice;
  ...
  public CatalysisKmc(Parser parser) {
    ...
    if (extraOutput2) {
      try {
        ...
        Logger.getLogger(CatalysisKmc.class.getName()).log(Level.SEVERE, null, e);
      }
      try {
   ...
   */
  public final void setLattice(CatalysisLattice lattice) {
      ...
      while (lattice.getCoverage() < maxCoverage) {
        if (lattice.isPaused()) {
          try {
            ...
            Logger.getLogger(CatalysisKmc.class.getName()).log(Level.SEVERE, null, ex);
          }
        } else {
  ...
  public void depositSeed() {
    ...
    if (justCentralFlake) {
      ...
      CatalysisAtom centralAtom = lattice.getCentralAtom();
      deposition:
      while (true) {
        depositAtom(centralAtom);
        for (int i = 0; i < centralAtom.getNumberOfNeighbours(); i++) {
          CatalysisAtom atom = centralAtom.getNeighbour(i);
          if (depositAtom(atom)) {
            depositedAtoms++;
          }
          if (depositedAtoms > 7) {
  ...
  private CatalysisAtom depositInIslands() {
      ...
      for (int i = 0; i < lattice.getHexaSizeI(); i++) {
        for (int j = 0; j < lattice.getHexaSizeJ(); j++) {
          CatalysisAtom atom = lattice.getAtom(i, j);
