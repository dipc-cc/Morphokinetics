...
public class GrapheneAtom extends Abstract2DDiffusionAtom {
    ...
    private static GrapheneTypesTable types_table;
    private byte n1, n2, n3;

    public GrapheneAtom(short X, short Y,HopsPerStep distance_per_step) {

        this.X = X;
        this.Y = Y;
        if (types_table == null) {
            types_table = new GrapheneTypesTable();
        }
...
//probabilidad de saltar a cierta posicion vecina
    private double prob_jump_to_neighbor(int origin_type, int pos) {
        ...
        Abstract2DDiffusionAtom atom = lattice.getNeighbor(X, Y, pos);
