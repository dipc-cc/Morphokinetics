...
print(totalRateEvents[-1]-totalRate[-1])
...
figR, ax = plt.subplots(1, figsize=(3.33*2,1.87*2))
figR.subplots_adjust(top=0.85,left=0.15,right=0.95,bottom=0.15)
t = list(totalRate[-1])[::-1]
#t.reverse()
#ax.plot(x, tgt, "o", label="target", color="red")
ax.plot(x, np.log(t)+10, "+-", label="TOF Rate", color=cm(4/20))
cm = plt.get_cmap('tab20')
#ax.plot(x, t, "+-", label="TOF Rate", color=cm(4/20))
markers=["o", "s","D","^","d","h","p"]
for i,a in enumerate(range(minAlfa,maxAlfa)):
    if any(abs(epsilon[-1,::-1,i]) > 0.005):
        #ax.plot(x, epsilon[-1,::-1,i], label=labelAlfa[a], color=cm(abs(i/20)), marker=markers[i%8])
        print(lastOmegas[-1,i]/rct[-1]*t[-1],rct[-1],t[-1])
        ax.fill_between(x, lastOmegas[:,i]/rct*(np.log(t)+10), label=labelAlfa[a], color=cm(a%20/(19)))
# ax2 = ax.twinx()
...
ax.set_ylabel(r"TOF (molecules/site/s)")
#ax.set_yscale("log")
