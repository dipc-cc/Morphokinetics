...
def diffusivityDistance():
    ...
    plt.loglog(time, diff/p.sizI/p.sizJ, label=r"$R^2 (\times \frac{1}{N_1 N_2})$",
               marker="s", ls="", mew=0, markerfacecolor=cm(0/8), alpha=0.8)
    plt.loglog(time, hops/p.sizI/p.sizJ, label=r"$N_h l^2 (\times \frac{1}{N_1 N_2})$",
               marker="p", ls="", mew=0, markerfacecolor=cm(1/8), alpha=0.8)
    plt.loglog(time, diff/hops, ".", label=r"$\frac{R^2}{N_h l^2}$",
               marker="H", ls="", mew=0, markerfacecolor=cm(2/8), alpha=0.8)
    plt.loglog(time, diff/time/p.sizI/p.sizJ/p.r_tt, label=r"$\frac{R^2}{t} (\times \frac{1}{N_1 N_2 r_{tt}})$",
               marker="o", ls="", mew=0, markerfacecolor=cm(3/8), alpha=0.8)
    plt.loglog(time, hops/time/p.sizI/p.sizJ/p.r_tt, label=r"$\frac{N_h l^2}{t} (\times \frac{1}{N_1 N_2 r_{tt}})$",
               marker="D", ls="", mew=0, markerfacecolor=cm(4/8), alpha=0.8)
    plt.loglog(time, diff/(parti*time*p.r_tt), label=r"$\frac{R^2}{Nt} (\times \frac{1}{r_{tt}})$",
               marker=">", ls="", mew=0, markerfacecolor=cm(5/8), alpha=0.8)
    plt.loglog(time, hops/(parti*time*p.r_tt), label=r"$\frac{N_h l^2}{Nt} (\times \frac{1}{r_{tt}})$",
               marker="*", ls="", mew=0, markerfacecolor=cm(6/8), alpha=0.8)
    #coverages
    cm = plt.get_cmap("Set1")
    plt.loglog(time, neg0/p.sizI/p.sizJ, label=r"$\theta_0$", color=cm(0/5), ls=(0,(1,1)))
    plt.loglog(time, neg1/p.sizI/p.sizJ, label=r"$\theta_1$", color=cm(1/5), ls=(0,(1,1)))
    plt.loglog(time, neg2/p.sizI/p.sizJ, label=r"$\theta_2$", color=cm(2/5), ls=(0,(1,1)))
    plt.loglog(time, neg3/p.sizI/p.sizJ, label=r"$\theta_3$", color=cm(3/5), ls=(0,(1,1)))
    if p.maxN == 6:
        plt.loglog(time, neg4/p.sizI/p.sizJ, label=r"$\theta_{4+}$", color=cm(4/5), ls=(0,(1,1)))
    plt.loglog(time, isld/p.sizI/p.sizJ, label="number of islands", color=cm(5/5))

    cm = plt.get_cmap("Set3")
    plt.loglog(time, fun.theta(time, p.flux), label=r"$1-e^{-Ft}$", color=cm(0))
    plt.loglog(time, cove, ".", color=cm(3/12), label=r"$\theta$", markerfacecolor="None")
