...
def annotate(ax):
    ...
    label = r"$\theta=0.30$"
    ax.annotate(label, xy=(0.075,0.4), xycoords="axes fraction",
...
def annotateBasic(ax):
    ...
    ax.annotate("", xy=(45,1e-2), xycoords='data', xytext=(45, 1e3),
                arrowprops=dict(arrowstyle="-", connectionstyle="arc3", ls="--", color="gray"))
    ax.annotate("", xy=(59,1e-2), xycoords='data', xytext=(59, 1e3),
                arrowprops=dict(arrowstyle="-", connectionstyle="arc3", ls="--", color="gray"))
    y = 2e-2
...
def plot(ax, data, i):
    ...
    if flux == 5e4:
        ...
        for i in range(len(rngt)-2,-1,-1):
            ...
            ax.text(xHalf,yHalf, text, color="black", bbox=bbox_props, ha="center", va="center", size=10, zorder=+400)
        i = 0 # print "interpolation" to the rest of lower x values
        ax.semilogy(x[0:rngt[i+1]+1], np.exp(fun.linear(x[0:rngt[i+1]+1], a, b)), ls=":", color="black", zorder=+300)
    if flux == 3.5e0:
