...
def addSurface(temperature, ax=0):
    try:
        fileName = glob.glob("/home/jalberdi004/mk_test/activationEnergy/basic/200/25.-surfaces/flux3.5e0/surface"+str(int(temperature))+".png")[0]
        im = Image.open(fileName)
        position = [0.16, 0.3, 0.3, 0.3]
        if ax != 0:
            position = [-0.1, 0.25, 0.3, 0.3]
...
def diffusivityDistance(smooth, binned, fig=0, ax=0, i=-1):
    ...
    up = 1e4
    lg, = ax.loglog(x, (np.sum(d.negs[0:7],axis=0))/p.sizI/p.sizJ*up, ms=1, lw=1, ls="-.", color="black", label=r"$\theta \cdot 10^5$")#, marker=markers[4])
    ...
    for k in range(0,4):
        ...
        ax.loglog(x, d.negs[k]/p.sizI/p.sizJ*up, label=label, ms=3, lw=1, ls="-", color=cm1(k/8))# marker=markers[k])
        #ax.text(x[1],1e1,"text")
        index = np.where(d.negs[k] > 0)[0][2]
        ax.text(x[index],d.negs[k][index]/p.sizI/p.sizJ*up, r"$\theta_{"+str(k)+r"}$", color=cm1(k/8), bbox=bbox_props)
    if smooth:
...
fluxes =  ["flux3.5e0"]#inf.getFluxes()
i = 0
for f in fluxes:
    temperaturesPlot = []
    print(f)
