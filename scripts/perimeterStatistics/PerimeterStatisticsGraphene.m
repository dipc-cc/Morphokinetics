function [Hist_angles,Hist_events,Hist_hops]=PerimeterStatisticsGraphene(NE,NRW,IntRadius)
  ...
  kind=1; % There are two types of particles in a graphene like lattice
  for rw=1:NRW %Index for each random walk
    ...
    for j=1:NE
      ...
      dx = -kind*ee1(k) + kind*c*ee2(k) + kind*c*ee3(k);
      dy =  0*ee1(k)    + s*ee2(k)      - s*ee3(k);
      kind = -1*kind; % change the type
